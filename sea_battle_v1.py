...
if __name__ == '__main__':
    ...
    players = []
    players.append(Player(name='SkillFactoryPlayer', is_ai=False, auto_ship=True, skill=1))
    ...
    while True:
        ...
        if game.status == 'in game':
            ...
            if shot_result == 'miss':
                ...
                try:
                    raise ValueError("Вы указали ту же самую координату!")
                except ValueError: game.current_player.message.append('Попробуйте еще раз!')
                continue
